...
本日はジョブメドレーでの全文検索エンジン導入に際して、調べたことや学んだことを紹介したいと思います！
...
弊社のジョブメドレーでの全文検索機能導入経験をもとに、全文検索エンジンの導入に際して、段階的な選択肢と判断基準をまとめました。
...
tsvector/tsquery が使用できるようですが、日本語の検索に対応する場合は、以下のようなプラグインが必要になるようです。
...
- **アナライザー**: トークナイザー + 各種フィルター
    - **正規化**: 大文字小文字統一、全角半角統一
    - **ストップワード**: 「の」「は」「を」などの除去
    - **ステミング**: 「running」→「run」のような語幹抽出
    - 類義語・表記揺れ: 「サーバー」「サーバ」「鯖」を同じ語として扱う 
...
明日の [MEDLEY Summer Tech Blog Relay](https://developer.medley.jp/entry/2025/08/15/20250815/) 10日目は井津さんです！ お楽しみに！
